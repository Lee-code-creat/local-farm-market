...
db.serialize(() => {
  ...
  db.run('DELETE FROM products', (err) => {
    ...
    const stmt = db.prepare(
      `INSERT INTO products (title, description, price, image_url, seller_id, purchase_count)
       VALUES (?, ?, ?, ?, ?, ?)`
    );

    products.forEach((p) => {
      const purchaseCount = randInt(0, 80); // random test value
      stmt.run(p.title, p.description, p.price, p.image_url, SELLER_ID, purchaseCount);
    });
